...
public class Service_Offer implements I_Service_Offer{
	...
	public List<Offer> afficher_Offers(Long studentId, int page, int size){
		...
		List<Offer> offers = (List<Offer>) offerRepo.findBySpecialization(std.getSpecialization(),pageable) ;
		return offers;
	}
	
	public List<Offer> afficher_Offers(Long studentId){
		Student std = stdRepo.findById(studentId)
                .orElseThrow(() -> new EntityNotFoundException("Student not found with id: " + studentId));
		List<Offer> offers = (List<Offer>) offerRepo.findBySpecialization(std.getSpecialization()) ;
		return offers;
	}
